    ...
    <div class="auth-page">
      <div class="auth-card card">
        ...
        @if (step === 'details') {
          ...
          <p class="subtitle">No password needed — we'll email you a code to verify</p>
          @if (error) { <div class="alert-error">{{ error }}</div> }
          <form (ngSubmit)="sendCode()">
            <div class="field">
              <label>Username</label>
              <input [(ngModel)]="username" name="username" placeholder="your_name" required autofocus />
            </div>
            <div class="field">
              <label>Email</label>
        ...
        @if (step === 'otp') {
          ...
          <div class="bottom-links">
            ...
            <button class="link-btn" (click)="step = 'details'; error = ''; resent = false">← Back</button>
          </div>
        }
...
export class RegisterComponent {
  ...
  sendCode(): void {
    this.error = '';
    this.loading = true;
    this.auth.register(this.email, this.username).subscribe({
  ...
  onInput(event: Event, index: number): void {
    ...
    input.value = val;
    if (val && index < 5) document.getElementById(`otp-reg-${index + 1}`)?.focus();
  }
  ...
  onPaste(event: ClipboardEvent): void {
    ...
    document.getElementById(`otp-reg-${Math.min(text.length, 5)}`)?.focus();
  }
}
